...
Our objective is to establish a theoretical framework that leverages the simplicity and intuitiveness of continuous attractors as an idealized abstraction of the slow manifolds locally.
We define "invariant sets" as regions within the neural space where the network consistently resides over time.
 Within these sets, we encounter "recurrent sets," comprising points in the neural space where any two points can be connected by a trajectory that remains entirely within the set, irrespective of the duration of the journey or the presence of noise.
Fast dynamics drive rapid system evolution, guiding it towards the recurrent set.
 Slow manifolds, within the context of neural networks, denote components where the system changes very slowly over time.
Lastly, we identify the asymptotic behavior, referring to the states that the network ultimately converges to, regardless of its initial state.
We show that in this framework, a continuous attractor can be approximated by different compositions of a slow manifold and either a suitable recurrent or an invariant set.

%two rings, one of which represents a recurrent set.

%On the highest level of abstraction of the dynamics of the system, we will  refer to the biologically irrelevant continuous attractors to explain brain function, but in this new framework these will be connected to the biologically relevant approximations the brain actually might use.
%We identify the parts of the neural space where the network stays inside for all future time (invariant sets).
%Slow manifolds in the context of neural networks refer to the aspects or components of the network that change or evolve very slowly over time.
%Asymptotic behavior: network converges to these states
% asymptotic behavior means that no matter where the network starts, over time, it will end up in a certain state or group of states
% in a neural network, asymptotic behavior refers to the network's tendency to evolve towards certain states over time, regardless of its initial state.
% fast dynamics refer to the components of the system that change or evolve quickly over time, these quick changes in the system are guiding it towards the recurrent set.
