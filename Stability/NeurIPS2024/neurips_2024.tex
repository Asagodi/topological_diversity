...
If a system is implementing such a computation, on a compact domain, the state will evolve to an attractor state inside the chain-recurrent recurrent set \citep{conley1978}.
...
Altough for well-trained networks this seperation of timescale exists, the analysis is not guaranteed to work for systems without a fast-slow decomposition.
...
For the bound discussed in Sec.\ref{sec:app:bouding}, we calculate the uniform norm of the found vector field $\|f\|_\infty = \sup f(\alpha)$.
