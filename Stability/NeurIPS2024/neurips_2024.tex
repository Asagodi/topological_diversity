...
\author{
    ...
    \texttt{\{abel.sagodi,guillermo.martin,piotr.sokol,memming.park\}@research.fchampalimaud.org} \\
}
...
We build on the persistent manifold theory to explain the commonalities between bifurcations away and approximations of continuous attractors.
...
In this study, we explore bifurcation away from and approximations of continuous attractors in theoretical models.
...
The proposed decomposition is general, and we be apply it to theoretical models and task-trained recurrent neural networks (RNNs) to reveal a ``universal motif'' of analog memory with various potential topologies.
...
Surprisingly, the number of fixed points is maintained throughtout a range of perturbation sizes (Fig.~\ref{fig:noorman_ring_allfxdpnts_allnorm}).
...
However, we have observed the ghost of the continuous attractors in many different systems (either through bifurcation or from finite-size effects): an \emph{attractive invariant manifold}.
Is this a universal phenomenon?
Moreover, although the exact ring attractor was not implemented, they were approximate ring attractors in the sense that the fixed points and connecting orbits still form a circle.
...
\begin{figure}[tbhp]
  \centering
  \includegraphics[width=.95\textwidth]{fastslow_decomposition_m}
...
%(B) 
%(C) C:\Users\abel_\Documents\Lab\Projects\topological_diversity\experiments\center_out\N128_T512_Tr100\tanh
%(D) C:\Users\abel_\Documents\Lab\Projects\topological_diversity\experiments\center_out\mask_after_saccade_N50_T500_noisy_actreg\tanh
...
Networks with different numbers of fixed points might have the same performance on the finite time scale (\(T_1\), bounded by \(T_1\uniformNorm{\varphi}\)) but have vastly different generalization properties because they differ in the number of fixed points (Fig.~\ref{fig:angular_loss}D).
...
If these four conditions hold, for example for task-trained RNNs, there exists a smooth function with a uniform norm matching the slowness on the manifold such that when added, it becomes a continuous attractor (Prop.~\ref{prop:revival}, see also Sec.~\ref{sec:near_ca}).
...
Unlike in most dynamical systems theories where the asymptotic behavior and topology of stability structures are studied, we expend an equal amount of focus on the effective finite time and asymptotic behavior.% of the ideal mathematical object.
...
%A future line of research could quantify the size of proposed homeostatic mechanisms to maintain analog working memory as proposed in e.g. \citep{Renart2003}.
...
If \(|f(\theta)|>0\) for all \(theta\), there is a circular flow with a variable speed.

where can have multiple roots, leading to fixed points. The nature of the flow around these fixed points can be classified into:

Stable fixed points: Points where the flow tends to as time progresses.
Unstable fixed points: Points from which the flow diverges.
...
\newpage
\section{A Near Perfect Analog Memory System must be close to a Continuous Attractor}
...
The  biggest eigenvalue (real part) for such a manifold needs to be much closer to zero than the second biggest.
We verify that this gap exists in the trained networks shown in Fig.~\ref{fig:fastslow_decomposition}.
The biggest eigenvalue is non-zero (the invariant manifold is not a continuous attractor), but it is removed from the second biggest, see Fig.~\ref{fig:eigenvalue_gap}.
\begin{figure}[tbhp]
     \centering
    \includegraphics[width=\textwidth]{eigenvalue_gap}
       \caption{ Fig~\ref{fig:fastslow_decomposition}).}
