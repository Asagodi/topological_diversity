...
\begin{figure}[tbhp]
  ...
  \caption{The different measures for memory capacity reflect 
  (A) 
  (B) asymptotic time}
...
All the trained networks can maintain memory of an angular variable, which implies that 
...
The mean accumulated error at the time at which the task was trained has a linear relationship with the number of fixed points (Fig.~\ref{fig:angular_loss}), which is another indication that the network relies on a ring invariant manifold to implement the task.
