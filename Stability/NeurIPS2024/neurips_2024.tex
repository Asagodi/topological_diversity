...
%Moreover, \(\manifold_\epsilon\) has \(\mathcal{O}(\epsilon)\) Hausdroff distance to \(\manifold_0\) and has the same smoothness as \(g\) and \(h\). 
\end{theorem}
...
\begin{equation}
  \begin{aligned}
	\vx_t &= \sigma(\win \vI_t + \vW \vx_{t-1} + \vb) + \zeta_t \label{eq:RNN:discrete}\\
	\vy_t &= \wout \vx_t + \bout
  \end{aligned}
\end{equation}
...
\(\sigma\colon \reals \to \reals\) an activation function that acts on each of the hidden dimensions,
...
\begin{align}
    \dm{\vx} &= -\vx\,\dm{t} + \sigma(\win \vI(t) + \vW \vx + \vb)\,\dm{t} + \sigma\,\dm{W}. \label{eq:RNN:continuous}
\end{align}
