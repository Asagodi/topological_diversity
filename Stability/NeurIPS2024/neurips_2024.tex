...
Introduce the noise term \(\zeta_t = \sigma \Delta W_t\), which represents the discrete-time noise.
...
\paragraph{LSTM}
The number of units the trained LSTMs was half of that of vanilla RNNs to match the number of paramters \citep{Hochreiter1997}.



\paragraph{GRU}
\citep{cho2014learning}
...
Adam optimization with \(\beta_1=0.9\) and \(\beta_2=0.999\) was employed with a batch size of 512 and training was run for 5000 gradient updates.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.9\textwidth]{davit}
...
DSA essentially requires 'learning' or fitting a separate model, which implicitly requires performing model selection with respect to the delay embedding, rank of the linear operator.
