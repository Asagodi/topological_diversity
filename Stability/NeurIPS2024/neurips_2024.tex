...
As the theory predicts, BLA bifurcates into a 1-dimensional slow manifold (dark colored regions) that contains fixed points and connecting orbits, and is overall still attractive.
...
Let $\manifold_\epsilon$ be a connected, compact, normally hyperbolic slow manifold  (as parametrized by Eq.\ref{eq:fenichel:flowtangent} and Eq.\ref{eq:fenichel:flownormal}).
...
This perturbation can be constructed by $\vp(x) = \int_{\manifold_\epsilon} - \delta(x-y)F(y)dy$, see for the proof Sec.~\ref{sec:supp:proofprop1}.
...
Hence, we have a continuous attractor at $\manifold_epsilon$.
\end{proof}
...
If we call $\hat{\alpha}_{\boldsymbol{\theta}}(\alpha_0, t) = g(\phi_{\bm{\theta}}(f(\alpha),t))$ we get the expression of the memory loss for this kind of memory as:
