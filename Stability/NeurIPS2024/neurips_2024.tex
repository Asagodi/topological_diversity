...
We observe that the bifircations from continuous attractors of theoretical models display various structurally stable dynamical structures with associated generalization characteristics.
Although their asymptotic behavior of memory are categorically distinct, their finite time behavior are similar and degreade gracefully.
Fast-slow decomposition analysis uncovers the persistent manifold that survives the seemingly destructive bifurcation.
Moreover, the recurrent neural networks trained on analog memory tasks display approximate continuous attractors with predictable slow manifolds and topologies.
...
Biological systems exhibit robust behaviors that requires neural information processing of analog variables such as intensity, direction, distance, and time intervals.
Virtually all models of working memory for continuous-valued information rely on persistent internal representations.
...
In biological neural networks, this vulnerability is well-known and often referred to as the ``fine-tuning problem''~\cite{seung1996,Renart2003,Park2023a}.
There are two primary sources of perturbations acting as noise in recurrent networks:
...
Thus, additional mechanisms are necessary to compensate for the degradation for particular implementations, often focusing on keeping the short-term behavior close to the continuous attractor case~\cite{Lim2012,Lim2013,Boerlin2013,Koulakov2002,Renart2003,gu2022}.
...
The distinct topology of these dynamics implies qualitatively different behavior in the asymptotic limit of time, and thus a corresponding pattern of error in temporal generalization.
By assuming normal hyperbolicity, we will separate the time scales to obtain fast flow normal to the ``ghost'' continuous attractor (a.k.a. slow manifold) and the slow flow within.
...
the universality statement asserts that the behavior of one-dimensional continuous attractors can be effectively described by slow ring manifolds.
...
%\begin{figure}[tbhp]
%  \centering
%  \includegraphics[width=\textwidth]{las}
%  \caption{Slow manifolds of line attractors
%}
%  \label{fig:las}
%\end{figure}
...
\end{align}
%
We consider systems with the ReLU, tanh, rectified tanh and sigmoid activation functions.
...
\begin{figure}[tbhp]
  ...
  The most often found solution is of the type \emph{fixed point ring manifold}.
}
