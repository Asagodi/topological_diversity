...
\subsection{Tasks}
memory guided saccade and the angular velocity integration tasks.

\subsection{Mapping out the found topologies}
%Fig 4
\begin{figure}[tbhp]
  \centering
 ...
 (A) An example fixed point type solution to the memory guided saccade task.
 (B) An example found solution to the angular velocity integration task.
 (C) 
...
We construct the one dimensional invariant manifold by fitting a cubic spline with periodic boundary constraints to these points (black line in Fig.~\ref{fig:fastslow_decomposition}A and B).
...
% although \citep{orhan2019diverse} considers many issues concerning how RNNs may hold memory: both sequential and nearly persistent solutions are part of a spectrum that emerges naturally in trained networks under different conditions
...
% 
 ...
 Previously, it has been observed that  fixed point analysis  has as a major limitation,  that the number of fixed points must be the equal across compared networks  \citep{maheswaranathan2019universality}.
