...
\begin{abstract}
Continuous attractors offer a unique solution for storing continuous-values in recurrent system states indefinitely.
...
We observe that the bifurcations from continuous attractors in theoretical models display various structurally stable dynamical structures, each with their associated generalization characteristics.
...
Virtually all neural models of working memory for continuous-valued information rely on persistent internal representations.
The continuous attractor structure in their recurrent dynamics are pivotal due to their ability to maintain activity patterns indefinitely through the neural population states~\cite{vyas2020,Dayan2001,Khona2022}.
...
Despite their widespread adoption as models of analog memory, continuous attractors are brittle mathematical objects, posing significant challenges.
...
Thus, additional mechanisms are necessary to compensate for the degradation for particular implementations, through bringing the short-term behavior close to the continuous attractor case~\cite{Lim2012,Lim2013,Boerlin2013,Koulakov2002,Renart2003,gu2022}.
However, we lack the theoretical basis for clinging to the brittle concept of continuous attractors for understanding biological working memory.
...
which reveal various structurally stable dynamical structures in the vicinity.
...
We derive a theory describing the persistent manifold that survives the seemingly destructive bifurcations and connects the different topologies as \textbf{approximate continuous attractors}.
...
\end{figure}
%replace ring attractor with N approx
...
Linearization on the attractor shows two eigenvalues, $0$ and $-2$, and perturbations cause the null eigenvalue to be non-zero and the line attractor disappears.
...
This phenomenon, wherein a continuous attractor is approximated by a manifold within the state space along which the drift occurs at a very slow pace, has been previously been commented on \citep{seung1997learning}.
...
\section{Theory of continuous attractor approximations}\label{sec:theory}
...
From the persistence theorem, we can describe the persistent manifold that survives the seemingly destructive bifurcations.
The theory implies that systems in the neighbourhood of a continuous attractor in the space of dynamical systems all share the same topology for the attractive persistent invariant manifold.
...
For higher dimensional, there is the possibility of chaos emerging, but we conjuncture that the bifurcations leading to such behavior are a measure zero set.
For all dimensions, there will the an attractive invariant manifold with the same topological structure and dimension as the original continuous attractor.
...
This allows us to  connect the different topologies as \textbf{approximate continuous attractors}.
...
The two  possible approximations of a ring attractor exhibit markedly distinct generalization characteristics.
...
\section{Fast-slow decomposition} %from trained RNNs
\subsection{Unpacking the slow manifolds in trained RNNs} %from trained RNNs
...
Recurrent neural networks (RNNs) are universal approximators of dynamical systems \citep{doya1993, schafer2006recurrent} and should therefore be able to approximate continuous attractors.
In this section, we dissecting the dynamics of RNNs by segregating time scales to delineate the rapid flow normal to the "ghost" continuous attractor, also known as the slow manifold, and the slow flow within.
We aim to identify the solution type of a trained RNN, and through this, to gain insights into its performance, error patterns, generalization capabilities, and proximity to a continuous attractor.
...
Through a series of experiments and analyses, we shed light on the intricate workings of neural networks when confronted with continuous-variable tasks
Amidst the variability of solutions, we observe a type of universality that aligns closely with our theoretical framework.
 This empirical evidence underscores the robustness and generalizability of our proposed theory of continuous memory.
...
private noise variable $\zeta$, and %$\zeta\sim \mathcal{N}(0, .001)$
...
We trained networks for every combination of the following parameters:
 ReLU, tanh, rectified tanh and sigmoid activation functions, 
 number of units/neurons (64, 128, 256), and
 activity regularization.
%
 All networks were trained using stochastic gradient descent with Adam with $\beta_1=0.9$ and $\beta_2=0.999$  for 5,000 iterations with a batch size of 32. % and training was run until no decrease in the loss occurred for 100 epochs




\subsection{Mapping out the found topologies}\label{sec:fastslowmethod}
...
We address this gap by investigating the internal mechanisms of neural network solvers for tasks involving continuous variables. Building upon prior work, which has shown networks' capabilities on such tasks \citep{cueva2019headdirection, cueva2021continuous}.
...
\end{figure}
  ...
  Less commonly found topologies are slow limit cycles (Fig.~\ref{fig:fastslow_decomposition}C) and a slow torus around a repulsive ring invariant manifold (Fig.~\ref{fig:fastslow_decomposition}D).
...
We construct the one dimensional invariant manifold $\manifold$ by fitting a cubic spline with periodic boundary constraints to these points (black line in Fig.~\ref{fig:fastslow_decomposition}A and B).
Alternatively, we identify the points along the trajectories that are the closest to set of point from the output space relevant to the task.
...
% 
% 
Previously, systemat analysis of the variability in network dynamics has been surveyed in vanilla RNNs and  variations in dynamical solutions over architecture and nonlinearity have been quantified \citep{sussillo2013blackbox, mante2013context, yang2019task, maheswaranathan2019universality,driscoll2022}. Furthermore, the spectrum of  how RNNs may hold memory has been addressed \citep{orhan2019diverse}.
...
For circular variables such as the goal-direction (e.g. for home navigation in bees \citep{frisch1993dance}) %home navigation + dance in bees: the angle of the waggle phase relative to gravity reflects the angle of the food source relative to the sun
%duration of the waggle phse reflects the distance to the food source from the hive
or head-direction, the temporal integration and working memory functions are naturally solved by a ring attractor (continuous attractor with a ring topology) \citep{kim2019generation, turner2017angular, turner2020neuroanatomical,hulse2020mechanisms}.
Continuous attractor models of the head-direction representation suggest that the representation emerges from the interactions of recurrent connections among neurons that form a ring-like structure~\citep{zhang1996,Noorman2022,ajabi2023,vafidis2022,boucheny2005continuous}.
...
We demonstrate the consequences of the Persistence Theorem in multiple models with a continuous ring attractor.

%for all ring attractors:
...
For very small networks the ring structure is destroyed.
Only the plane persist as a slow manifold.
...
\subsection{Sensitivity of continuous attractors to perturbations}

%We will show that there are differences at how well approximations perform at different timescales.

We will assess performance at two different time-scales,  asymptotic and finite time.
...
We measure how performance of different models for the representation of an angular variable drop as a function of perturbation size Fig.~\ref{fig:performance}.
...
We measure the error on the asymptotic timescale as follows.
...
Although the corresponding configurations are measure zero, we showed that  when the invariant manifold theorem holds, the finite time behavior of the trajectories and the gradient through time only slowly break down.
...
\begin{align}
T_{stim} \sim \mathcal{U}(50, 500) 
T_{response} \mathcal{U}(50, 500)
\end{align}
...
Velocity at every timestep %(assumed to be 100 ms)
 is sampled from as a Gaussian Process for smooth movement trajectories, consistent with the observed animal behavior in flies and rodents.
...
with the dimensionless parameter $J^-$ representing the strength of the weak crossdirectional connections, $J^+$ the strength of the stronger isodirectional connections,
 ...
 \citep{redish1996coupled} %Equation 9,12
