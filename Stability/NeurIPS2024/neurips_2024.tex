...
\section{A critique of pure continuous attractors}
%par4: observation: CAs bifurcate into invariant manifolds topologically equivalent to the original CA
%Fig1

Continuous attractors are not only structurally unstable, they bifurcate almost certainly for an arbitrary perturbation.
Examples abound.
However, interestingly, continuous attractors seemingly tend to bifurcate into invariant manifolds topologically equivalent to the original continuous attractor.
...
\mpcomment{We need to justify why we are in this general ODE/RNN setting if we care about neuroscience.}
...
This phenomenon, wherein a continuous attractor is approximated by a manifold within the state space along which the drift occurs at a very slow pace, has previously been commented on \citep{seung1997learning}.
...
\mpcomment{Shall we move the other biologically relevant systems here?}
...
Continuous attractor models of the head-direction representation suggest that the representation emerges from the interactions of recurrent connections among neurons that form a ring-like structure~\citep{zhang1996,Noorman2022,ajabi2023,vafidis2022,boucheny2005continuous,knierim2012}.
...
A well-established way to create a continuum of fixed points on a ring is to use a circulant matrix \citep{samsonovich1997path, seeholzer2017efficient,redish1996coupled,goodridge2000,compte2000synaptic}.
...
%
Furthermore, our theory indicates that the reason why networks of generic spiking neurons with connectivity inferred from light-resolution microscopy gives rise to approximate ring attractors\citep{kakaria2017}, is becuase they are nearby ring attractors as dynamical systems.
...
\mpcomment{nope. not machine learning audience! right?}
...
\mpcomment{probably keep it general, and not say RNN.}
...
From the persistence theorem \mpcomment{first mention in this form}, we get a description of the persistent manifold that survives the seemingly destructive bifurcations.
...
For example, for a torus the possible slow dynamics are described by \mpcomment{partial sentence}
For higher dimensional \mpcomment{manifolds?}, there is the possibility of chaos emerging, but we conjuncture that the bifurcations leading to such behavior are a measure zero set.
For all dimensions, there will be an attractive invariant manifold with the same topological structure and dimension as the original continuous attractor.
...
\section{Continuous attractor approximations} %from trained RNNs
...
\mpcomment{you can't start a section with an equation.}
\begin{equation}
  \begin{aligned}
...
We trained networks for every combination of the following parameters:
 ReLU, tanh and softplus activation functions, 
 number of units/neurons (64, 128, 256), and
 activity regularization ($10^{-3}, 0, 10^{3}$).
%
All networks were trained using stochastic gradient descent with Adam with $\beta_1=0.9$ and $\beta_2=0.999$  for at most 5,000 iterations with a batch size of 256 and a stopping criterion............. % and training was run until no decrease in the loss occurred for 100 epochs

To avoid overly complex solutions that do not generalize well for longer trials, we penalized high activity using an L2 regularization on $x_t$.
...
Less commonly found topologies include slow limit cycles (Fig.~\ref{fig:fastslow_decomposition}C) and a slow torus around a repulsive ring invariant manifold (Fig.~\ref{fig:fastslow_decomposition}D).
 These solutions are consistent with both observations of uses of the usefulness of non-persistent \mpcomment{not clear} for memory storage \citep{hirsch1995computing,Park2023a} and neuronal circuits underlying persistent representations despite time varying activity \citep{druckmann2012neuronal}.
...
Alternatively, we identify the points along the trajectories that are the closest to set of point from the output space relevant to the task after convergence.
...
The universal structure of continuous attractor appriximations as slow invariant manifolds allows us to connect the different topologies as \textbf{approximate continuous attractors} (Sec.~\ref{sec:attractor_bif}).
...
\section{Generlalization properties}
...
%\subsection{Loss of function: Sensitivity of continuous attractors to perturbations}
...
\section{For Abel's paper}

$\boldsymbol{\alpha} \in \mathcal{U} \subset \mathbb{R}^D$ with distance $d(x,y): \mathcal{U} \times \mathcal{U} \rightarrow \mathbb{R}$. Then we have a dynamical system $\bm{x} \in \mathbb{R}^N$ with autonomous dynamics $\dot{ \bm{x}} = F_{\boldsymbol{\theta}} (\bm{x})$. There is an encoder mapping $f: \mathcal{U} \rightarrow \mathbb{R}^N$ such that the inverse of this encoder mapping is a function (although not necessarily injective): $f^{-1}: Im(f) \rightarrow \mathcal{U}$. Then, we define our memory loss in a time $T$ as:
...
Let's assume that we have managed the system $F_{\boldsymbol{\theta}}$ to be 'good', i.e. have a slow manifold $\mathcal{M} \in \mathbb{R}^N$ in bijection with $\mathcal{U}$, i.e. $f|_{\mathcal{M}}$ is not a mapping but a bijective function and:

\begin{equation}
\forall \bm{x} \in \mathcal{M} \quad \dot{\bm{x}} = \epsilon_{\bm{\theta}}(\bm{x})\frac{\frac{\partial f}{\partial \alpha}(f^{-1}(\bm{x}))}{||\frac{\partial f}{\partial \alpha}(f^{-1}(\bm{x}))||}
\end{equation}
...
Velocity at every timestep %(of  to be 100 ms)
