...
%All networks were trained using stochastic gradient descent with momentum \citep for 5,000 iterations with a batch size of 32.

We trained vanilla RNNs with a ReLU or tanh nonlinearity for the recurrent layer and a linear output layer on the memory guided saccade and the angular velocity integration tasks.
 All networks were trained using stochastic gradient descent with Adam with $\beta_1=0.9$ and $\beta_2=0.999$  with a batch size of 32. % and training was run until no decrease in the loss occurred for 100 epochs
...
\begin{figure}[tbhp]
  ...
  \label{fig:fastslow_decomp}
\end{figure}



  Fig.~\ref{fig:fastslow_decomp}
...
We construct the one dimensional invariant manifold by fitting a cubic spline with periodic boundary constraints to these points (black line in Fig~\ref{fastslow_decomp}A and B).
Fintally, we find the stable fixed points through a threshold on 
