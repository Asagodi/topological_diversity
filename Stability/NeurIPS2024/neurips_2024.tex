...
Our theory shows  that normally hyperbolic continuous attractors are robust to perturbations. 
This has the implication that attractive slow manifolds are general approximations of continuous attractors and are the only ones for linear decoders.
...
%CA
Continuous attractors are prevalent in theoretical neuroscience as tools to model neural representation and computation ranging from internal representations of head direction \citep{Skaggs1995, goodridge2000} and eye positions \citep{seung1996} to perceptual decision-making and working memory dependent on continuous variables~\cite{Khona2022}.
%QPTA
...
%We will describe some continuous attractors which have such neighbourhoods.
...
We can think of the finite size realization as a small perturbation to the infinite size network.
