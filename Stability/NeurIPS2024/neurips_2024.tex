...
NNs are universal approximators of dynamical systems \citep{doya1993}, can we 
In this section, we delineate a novel approach to dissecting the dynamics of recurrent neural networks (RNNs) by segregating time scales to delineate the rapid flow normal to the "ghost" continuous attractor, also known as the slow manifold, and the sluggish flow within.
...
\begin{equation}
  \begin{aligned}
	\vx_t &= f(\win \vI_t + \vW \vx_{t-1} + \vb) +  \sigma \eta(t)\label{eq:RNN:discrete}\\
	\vy_t &= \wout \vx_t + \bout
  \end{aligned}
\end{equation}
...
\begin{align}
    \dot{\vx} &= -\vx + f(\win \vI + \vW \vx + \vb). \label{eq:RNN:continuous}
\end{align}
