...
Assumptions:
- \(\manifold_X= \manifold_{\text{slow}}\times \manifold_{null}\) with \(\manifold_{slow}\simeq \manifold_Y\)
...
Further assume that this manifold can be decomposed \(\manifold_X= \manifold_{\text{slow}}\times \manifold_{null}\) with \(\manifold_{slow}\simeq \manifold_Y\) and that 
...
Furthermore, the trajectories of the second system make up a vector bundle for which the output projection is the bundle projection.
\end{theorem}

In other words, after applying this perturbation we get \(\dot x|_\text{slow} = 0\).
...
This would imply that the invariant manifold \( \manifold_{\text{slow}}\times \manifold_{null}\) hold for every point because \(\manifold_X\) is a manifold
