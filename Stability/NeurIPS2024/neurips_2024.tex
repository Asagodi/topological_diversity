...
In the case of the ring attractor, the memory we would like to encode is $\alpha \in \mathcal{U} = [0,2 \pi)$, the error is defined as $|x-y| = o_\pi(|x-y|)$ where:
\begin{equation}
    o_\pi(x) = \begin{cases}
    x & \text{if } x < \pi \\
...
\begin{equation}
    \mathcal{L}(T) =  \frac{1}{2 \pi} \int_0^{2 \pi}   o_\pi \left( \left| \hat{\alpha}_{\boldsymbol{\theta}}(\alpha_0, t) - \alpha_0  \right| \right)  d\alpha_0
\end{equation}
