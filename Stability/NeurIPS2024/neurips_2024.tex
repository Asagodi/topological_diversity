...
\ascomment{todo: finding fixed points}
...
\begin{align}
 J(\vx_i)\vd\vx_i = x_i
\end{align}
where $J(\vx_i)$ is the Jacobian of the system at $\vx_i$ and $\vx_{i+1}=\vx_i-\vd\vx_i$.
We initialize $x_0$ on the invaraint ring uniformly.
 ...
 % inv man
   ...
   \begin{figure}[tbhp]
    ...
    \includegraphics[width=\textwidth]{im_all_last2}
       \caption{Representative identified invariant manifolds (projected onto the output space, in black) with the fixed points (cyan for stable, orange for saddle and black for unstable). The reference target ring is shown in grey.}
         \label{fig:im_rep}
\end{figure}
