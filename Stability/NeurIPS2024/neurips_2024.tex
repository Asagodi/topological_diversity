...
\citep{miller2006analysis}
\citep{wu2008dynamics}
\citep{fung2010}


%infinite network size limit
frozen stabilisation, which allows a family of neural networks to self-organise to a state exhibiting memory manifolds and a wide range of relaxational timescales. The principle works by spontaneously freezing/slowing the dynamics of a (random) subpopulation of neurons, thereby creating a static background input which serves to stabilise the remaining population. \citep{can2021emergence}
...
In the 1D case, it is well known that the presence of such synaptic heterogeneity causes a drift of an input-induced activity pattern to one of a finite number of attractor positions which are randomly spread over representational space \citep{Renart2003,itskov2011}S.
...
\subsection{Other fitting methods}
