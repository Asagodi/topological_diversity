...
% We will demonstrate that this bound can be successfully found in trained RNNs numerically.
...
%\subsection{Generalization properties of continuous attractor approximations}\label{sec:generalization:sub}
...
A manifold of equilibria in a continuous attractor network provides continuous memory, making it a compelling model in theoretical neuroscience. 
However, continuous attractors almost certainly bifurcate under arbitrary perturbations except in special paramtric forms, making them unsuitable as models for neural computation in the traditional perspective on robustness.
We will therefore never see we reach through traditional RNN training techniques and never observe a biological one.
Nevertheless, have shown that continuous attractors are functionally robust and therefore are an important concept to describe the neural computation underlying analog memory.
...
There was a diverse set of bifurcations all sharing an essential element: an attractive invariant slow manifold.
%
%as we have shown...	
...
As the theory predicted, trained networks have an attractive ring invariant manifold, which allows for a more useful comparison between trained networks than existing fixed point analyses.
This supports our theory that theoretical models that satisfy conditions \ref{converse:bijection}-\ref{converse:Dtype} %and hence have behavior consistent with an almost perfect memory system, 
will be near continuous attractors (in parameter space).
...
%limitations
Although, we only explicitly describe the topology and dimensionality of the identified invariant manifolds for a representative set,
 the results indicate that most solutions have a ring invariant manifold with a slow flow on it.
Although this separation of timescale necessarily exists for well-trained networks, the analysis is not guaranteed to work for systems without a fast-slow decomposition.
%point above
...
%The second assumption we need is about the speed along the output manifold.
...
We need to assume that there is a slow flow (in the direction of \(\manifold_\text{slow}\)). %(it is possible that there is no flow in output space but there is flow in the kernel of the output mapping)
...
%equiv.: ---- the supremum over the uniform norm of the vector field tangent to sections in the space of smooth sections of the fiber bundle 
%maximum perturbation


%eta: 
% a smooth map φ : M → N and a vector field X on M
%||\dot x|N||\infty Where we take the component N normal to each fiber \g^{-1}(y)
%for later: eta in output space: average vector field tangent to output manifold per fiber
...
The fibers are unions of trajectories that are all mapped onto the same value in \(\manifold_{\text{output}}\). %are equivalence classes of the output projection 
