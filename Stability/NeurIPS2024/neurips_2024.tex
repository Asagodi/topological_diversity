...
However, interestingly, after bifurcation, continuous attractors seemingly tend to leave a ``ghost'' manifolds topologically equivalent to the original continuous attractor (note the slow speed).
Furthermore, the flow field is not escaping the ghost manifold, i.e., it is an invariant manifold.
This phenomenon, wherein a continuous attractor is approximated by a manifold within the neural space along which the drift occurs at a very slow pace, has previously been commented~\citep{seung1997learning,Mante2013-em}.
...
A well-established approach, consistent with the principles observed in the fly head direction network \citep{kakaria2017}, involves utilizing a circulant matrix as the connectivity matrix\citep{samsonovich1997path, seeholzer2017efficient, redish1996coupled, goodridge2000, compte2000synaptic}.
...
The fitted models through this method also contain an invariant ring manifold that on which the dynamics contains stable and saddle fixed points.

\subsection{Observations}
\mpcomment{to extend the summary and provide outlook}
\begin{enumerate}
    \item biological models of ring attractors are brittle.
    \item they bifurcate into many different topologies.
    \item Are continuous attractors still useful as a model of how animals might represent continuous variables?
    \item we observe ghost manifolds. Is it a universal phenomenon?
\end{enumerate}
We seek implementation-agnostic answers.
...
To describe an arbitrary bifurcation of interest, we introduce a sufficiently smooth function $g$, such that the following system is equivalent to the original ODE:
...
Consider a behaviorally relevant time scale for analog working memory, say 10 ms to 10 sec range.
If the flow is orders of magnitude slower, for example, 1000 sec or longer, its effect is too slow to have practical impact on the behavior that relies on the working memory mechanism.
This clear gap in the fast and slow time scales can be recast as \emph{extended normal hyperbolicity} of the slow manifold by relaxing the zero real part to separation of time scales (reciprocal of eigenvalues or Lyapunov exponents).
In other words, the attractive flow normal to the manifold is uniformly faster than the flow tangent to the manifold.

By taking the limit of the slow flow to infinity, we achieve the reversal of the persistent manifold theorem.
\begin{theorem}[Revival of continuous attractor (informal)]
...
There exist a perturbation with uniform norm at most $\eta$ that induces a bifurcation to revive the continuous attractor manifold.
\end{theorem}
\mpcomment{can we actually make this a theorem?}

We can bound the memory performance on the short-time scale performance based on the uniform bound of the slow flow.
...
It is expected that working memory tasks generalize to longer durations~\citep{Park2023a}.
Long-time scale behavior of the slow manifold is dominated by the stability structure, i.e., the topology of the dynamics.
Although we have seen numerous topologies in Sec.~\ref{sec:critique}, they are fundamentally limited, especially in low-dimensions.
...
\mpcomment{how does KAM help us?}
...
\begin{figure}[tbhp]
  ...
  (E) Average accumulated angular error over time for two networks that have comparable angular errors at $T_1$, but have vastly different generalization properties because they differ in the number of fixed points.
  }
...
The networks perform well on the timescale on which they have been trained $T_1=256$ time steps.



%uniform bound
%infinite norm
This loss is bounded by the uniform norm of the vector field  on the invariant manifold (See Sec.Supp.~\ref{sec:supp:vf} and \ref{sec:app:bouding}).
...
The mean accumulated error at the time at which the task was trained has a linear relationship with the number of fixed points (Fig.~\ref{fig:angular_loss}A).
This is another indication that the networks rely on a ring invariant manifold to implement the task.

%(Fig.~\ref{fig:angular_loss}A and B)
