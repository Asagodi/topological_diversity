...
Animal behavior often showcases remarkable abilities to navigate and interact with their environment, even in the absence of sensory cues.
 This suggests the existence of sophisticated internal representations that help them understand variables like location and direction.
For instance, rodents demonstrate impressive spatial memory and navigation skills within intricate maze environments, indicating a robust internal representation of space.
The computation through dynamics (CTD) framework provides a theoretical lens through which we can understand how complex behaviors, such as animal navigation, emerge from the dynamics of neural systems. In the context of animal navigation, the CTD framework suggests that the brain processes sensory information and generates behavioral outputs through dynamic interactions within neural circuits.
Within this framework, the internal representations of key variables like location and head direction emerge from the dynamics of neural activity.
...
Recurrent Neural Networks (RNNs) provide a computational model that captures some of the key principles of computation through dynamics (CTD) observed in neural systems. In the context of animal navigation and internal representation of variables like location and direction, RNNs offer a framework for understanding how neural dynamics give rise to complex behaviors.
...
%    Phase portraits for unbounded and bounded line attractors~\eqref{eq:TLN}.
...
This phenomenon has been previously noted, wherein a continuous attractor is approximated by a manifold within the state space, along which the drift occurs at a very slow pace  \citep{seung1997learning}.
%``a manifold of stable fixed points'' \citep{Seung2000}.
An invariant ring manifold with fixed points on it has been observed in finite dimensional low-rank attractors which converge to a ring attractor in the infinite unit limit \citep{mastrogiuseppe2018, beiran2021}.
Similarly, this has been observed in ring attractors with a finite number of units \citep{Noorman2022}.
...
\subsubsection{Unpacking the slow manifolds in trained RNNs} %from trained RNNs
