...
From the persistence theorem, we get a description of the persistent manifold that survives the seemingly destructive bifurcations.
...
For example, for a torus the possible slow dynamics are described by 
...
integration of evidence for continuous perceptual judgments, e.g. a hunting predator needs to compute the net direction of motion of a large group of prey \citep{esnaola2022flexible}.
...
\section{Continuous attractor approxiamtions} %from trained RNNs
...
To avoid overly complex solutions that don't generalize well, we penalized high activity using an L2 regularization on $x_t$.
...
and (2) perform a memory guided saccade task for a ring variable.
...
Less commonly found topologies include slow limit cycles (Fig.~\ref{fig:fastslow_decomposition}C) and a slow torus around a repulsive ring invariant manifold (Fig.~\ref{fig:fastslow_decomposition}D).
 These solutions are consistent with both  observations of uses of the usefullness of non-persistent for memory storage \citep{hirsch1995computing,Park2023a} and neuronal circuits underlying persistent representations despite time varying activity \citep{druckmann2012neuronal}.
...
We assess the intrinsic dimensionality of the manifold with the Method Of Moments algorithm \citep{amsaleg2018extreme}.
...
It is interesting to consider how structural constraints would affect the representation and computation in the trained RNNs.
Future work should systematically explore this issue.
