...
($\{10^{-10}\}_{i=4,\dots 10}$)  based on best performance of the task, measured as mean last ten MSE values averaged over ten runs.
 The slow manifold that is created after perturbations is provides gradients that can counteract parameter diffusions for all networks (on short trials), even for the ones that have the potential for exploding gradients ( Fig.~\ref{fig:maintenance_h0}A and D).
 %new conclusions
%The distribution of MSE for the task can be used as proxy for the misadjustment from the optimal solution.
...
(D) MSE distribution during learning and in the presence of noise. Learning counteracts learning in all three networks.
(E) Closeness of the parameters to the original while learning with noise (right) and without learning (right).
...
\begin{figure}[H]
    ...
    \includegraphics[width=\textwidth]{speeds}
       \caption{Speeds along the invariant manifold during learning. }
         \label{fig:speeds}
\end{figure}
