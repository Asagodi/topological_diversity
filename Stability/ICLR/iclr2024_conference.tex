...
These conditions are met (up to numerical precision) for the BLA example in Fig.~\ref{fig:ublabla}B (see Supp. Sec. \label{sec:supp:fast_slow_form} for the rerparametrization of the BLA to this form).
...
To illustrate the effect of bifurcations from continuous attractor solution, we take a 1-dimensional slice of the loss surface, see Fig.~\ref{fig:maintenance}D.
Specifically, we continuously vary one of the entries of the self-recurrent connection matrix:
\begin{align}
    \vW_{1,1} \leftarrow \vW_{1,1} + \Delta \quad \text{  with } \Delta\in[-\tfrac{1}{10},\tfrac{1}{10}].
\end{align}
...
As can be seen in Fig.~\ref{fig:maintenance}D, for UBLA and iRNN, $\Delta > 0$  perturbations shows exponentially increasing loss and corresponds to an exploding gradient dynamical system.
...
The perturbations that leave the line attractor intact or to lead to a system with two fixed points have measure zero in parameter space.
...
The (local, in the case of the single fixed point) invariant manifold manifold is indeed persistent for the BLA and homeomorphic to the original (the bounded line).



\subsubsection{Maintaining a neural integrator}\label{sec:exp:maintaining}
To investigate the differential effect of stoachstich gradient descent (SGD) on the three neural integrator models, we performed three learning experiments using the continuous-valued click integration task.
...
For the SGD, the last output of the network after $T$ steps was taken to calculate the loss based on the mean squared error (MSE) over a batch of 1024 trials. A gradient step was then taken with a fixed gradient step size $\lambda$ (the learning rate). We set $\alpha=1$ and $\beta=20$ in Eq \ref{eq:ubla} and \ref{eq:bla}.
...
Fig.~\ref{fig:nonmaintenance}C shows that for all networks, gradient descent (with constant learning rate, chosen from a grid search) was able to counter the diffusion.
...
The invariant manifold of the BLA is persistent throughout learning in many cases, see Supp. Fig. \ref{sec:supp:learning} and \ref{fig:vfs1}. However, the gradients are not pointing towards the BLA but to one of the bifurcations of the BLA (see Supp Fig. \ref{fig:wdn_mse_trajectories_F1}, Supp. Fig. \ref{fig:vfs5} and Supp. Fig. \ref{fig:vfs30}).
iRNN often finds a different optimum (it settles at a part of the state space that is at a non-zero distance from the initial recurrent matrix and bias (Fig. \ref{fig:maintenance} and Supp. Fig \ref{fig:vfs1}).
...
\begin{figure}[thbp]
  \centering
  \includegraphics[width=\textwidth]{nonmaintenance.pdf}
  \caption{
...
(B) Gradient distribution at the beginning and end of trials.
(C) MSE distribution during learning and in the presence of noise. Learning counteracts learning in all three networks.
(D) Closeness of the parameters  to the original  while learning with noise.
(E) All networks converge to local non-optimal solutions after perturbations.
  }
  \label{fig:nonmaintenance}
\end{figure}
...
We calculated the loss on a batch of inputs for various noise levels for all three noise types (Supp.~Fig.~\ref{fig:matching_noise_3types_cont}).
...
In the presence of input and internal noise the UBLA has a higher tendency to have exploding gradients for lower learning rates, see Supp.~Fig.~\ref{fig:all_lrs_vs_mses}. 
...
Gradient distributions for all noise levels and learning rates. For some noise levels a bimodal distribution can be seen for the LAs, but exploding gradients seem to be absent for iRNN for low learning rates (Fig. \ref{fig:nonmaintenance}B).
The fact that the iRNN has a smaller range of gradients explains exploding gradients at a higher learning rate for iRNN and the need for a higher learning rate for optimal maintenance of the solution and is explained by a more shallow loss landscape in the close vininity of the iRNN (Fig.~\ref{fig:nonmaintenance}D).
...
($\{10^{-8},10^{-9},10^{-10}\}$)
 based on best performance of the task of 10 runs.

%new conclusions
%The distribution of MSE for the task can be used as proxy for the misadjustment from the optimal solution.
 We further show that all networks converge to a different, non-optimal, solution as they settle in a regime in parameter space that has a higher norm difference with the initial parameters of the neural integrator Fig.~\ref{fig:nonmaintenance}D and E.
 We conclude therefore that, in practice, it is difficult to maintain all 3 attractors.
...
We evaluate the  the effect of parameteric perturbations of the form $ \vW \leftarrow \vW + \vV$ with $\vV_{i,j}\iidsample\mathcal{N}(0,\frac{1}{100})$ on a network of size $N = 6$ with $J_E^*= 4$ and $J_I=-2.4$ through~\ref{sec:supp:ring_perturbations}.
...
The probability over the bifurcations is numerically estimated.
