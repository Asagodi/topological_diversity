...
To illustrate the effect of bifurcations from continuous attractor solution, we take a 1-dimensional slice of the loss surface, see Fig.~\ref{fig:maintenance_h0}B.
Specifically, we continuously vary one of the entries of the self-recurrent connection matrix: $    \vW_{1,1} \leftarrow \vW_{1,1} + \Delta \quad \text{  with } \Delta\in[-\tfrac{1}{10},\tfrac{1}{10}].$
...
For the matched noise level, we find the optimal learning rate for all each network separately.
...
%The distribution of MSE for the task can be used as proxy for the misadjustment from the optimal solution.
 ...
 We conclude therefore that, in practice, it is difficult to maintain any of the 3 attractors. 
Finally, the slow manifold that is created after perturbations is helpful to counteract parameter diffusions for all networks (on short trials), even for the ones that have the potential for exploding gradients.
...
\subsection{Perturbation of a continuous attractor}\label{sec:hd} % Finite-sized head direction network models
