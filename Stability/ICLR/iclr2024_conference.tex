...
We then chose the corresponding noise level per integrator that corresponded to the average loss. For the noise in the weights the average was taken over 200 weight perturbations.
...
Compact, boundaryless invariant manifolds are both forward and backward invariant, i.e. they are invariant \citep{wiggins1994}.
...
\begin{figure}[H]
     \centering
  \includegraphics[width=.7\textwidth]{N6_ring_perturbations}
       \caption{Perturbations to the ring attractor introduced in \cite{Noorman2022}.
       }
         \label{fig:Noorman_ring_activity_pert}
\end{figure}


We evaluate the effect of perturbations on a network of size $N = 6$ with $J_E^*= 4$ and $J_I=-2.4$ through local and parametrized perturbations. %, similarly as for the toy ring attractor model.
...
We further investigate the effect of parameteric perturbations of the form $ \vW \leftarrow \vW + \vV$ with $\vV_{i,j}\sim\mathcal{N}(0,\frac{1}{100})$.
...
\subsection{Bifurcation analysis of the line attractors}

\subsubsection{Discrepancies between discrete and continuous RNNs}
...
\subsubsection{Unbounded line attractor}
\label{sec:ubla}
...
\subsubsection{Bounded line attractor}\label{sec:supp:bla}
...
%2 fixed points
If condition 1 is violated, but condition 2 is satisfied with exactly oner strict inequality, there are two fixed points on the boundary of the admissible quadrant.
...
It is not possible to have a line attractor with a fixed point off it for as there cannot be disjoint fixed points that are linearly dependent \citep[Lemma 5.2]{morrison2016a}.
\end{proof}
...
\subsection{Heading direction network}
...
\subsection{Persistence in non-differentiable vector fields}
