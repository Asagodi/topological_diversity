...
For the matched noise level, we find the optimal learning rate for all three networks.
The optimal learning rates for the input-type noise experiments were chosed from a set of values 
($\{10^{-10}\}_{i=4,\dots 10}$)  based on best performance of the task, measured as mean last ten MSE values averaged over ten runs.
...
Gradient distributions for all noise levels and learning rates. For some noise levels a bimodal distribution can be seen for the LAs, but exploding gradients seem to be absent for iRNN for low learning rates (Fig. \ref{fig:maintenance_h0}B).
