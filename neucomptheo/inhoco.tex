...
\newpage
\section{Justifications and restrictions}\label{sec:jar}
...
We define the implemented (neural) computation as an input-output mapping for times $t\in T$ defined by the function $\varphi:\reals^\reals\rightarrow\reals^K$:\footnote{We discuss some restrictions on the type of mapping we consider in Sec.~\ref{sec:jar}.}
\begin{equation}
o(t) = \varphi(\{u(t)\}_{t\in T})
\end{equation}
where $o(t)$ is the target output for each time point $t$ and  $u(t)$ is the input.
