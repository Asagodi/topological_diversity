...
\end{equation}
where $o(t)$ is the target output for each time point $t\in T$ and the input  u:\mathbb {R} _{+}\to \mathbb {R} ^{m}}  is a Lebesgue measurable essentially bounded external input and 
$f$ is a Lipschitz continuous function w.r.t. the first argument uniformly w.r.t. the second one
