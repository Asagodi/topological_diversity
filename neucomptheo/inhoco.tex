...
For asymptotic dynamics one can formulate a surjective mapping to finite-state machines if MSs are fixed points or one restricts what effects the input can have.
...
\section{FSM}\label{sec:fsm}
\begin{theorem}
If the sinks (final nodes in the DAG representation of the Morse Decomposition) of finest Morse decomposition is composed of fixed points, then there exists a surjective mapping to deterministic finite state machines.
\end{theorem}
