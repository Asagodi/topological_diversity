...
\newpage
\section{Background}%{Relevant literature}
...
What we get is a system that can be mapped to a finite state machine. %an extension thereof
...
``fast'' compared to autonomous dynamics




\paragraph{IO mapping}
Smooth
...
\paragraph{RNN behavior simplifications}
\paragraph{Echo State Property (ESP)}
the RNN internal state at present time must be uniquely determined by the whole history of the innite past input sequence that we used to drive the RNN with

no fading memory

\paragraph{Abstracting  out time}
