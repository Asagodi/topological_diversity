...
The space of all dynamical systems can be partitioned the equivalence of their qualitative behavior~\citep{Arnold1994}, abstracting away the finite time transient behaviors and the details of the vector field.
...
\begin{align}
    ...
    \end{dcases}
\end{align}
Poincar\'e introduced the notion of homeomorphism $\homeo: \inputSpace \to \inputSpace$, a continuous invertible map from the trajectories (solutions) of one system to the other.
If there exists such a homeomorphism, then the two systems are \emph{topologically conjugate} (Def.~\ref{def:top_conj}).
...
Theoretical neuroscience and machine learning have extensively used these as building blocks to help us expand our intuition on neural computation.
...
\end{align}
for some $\epsilon > 0$ does not break the topological conjugacy, then it is said to be \emph{structurally stable}.
...
(1) a small distance in the trajectories after a continuous coordinate transform,
and (2) a small complexity continuous coordinate transform.
...
\end{align}
For a finite time horizon $[0, T_\text{max}]$ and assuming a Lipschitz bound $L$ for $\vf(\cdot)$ in the relevant domain, a uniform bound on $\norm{\vDelta(\cdot)}$, we can bound the trajectory deviation from the unperturbed system.
...
\end{align}
for any norm $\norm{\cdot}$.
Thus, for small additive perturbation, the trajectories are robust for a short time.
\begin{align}\label{eq:d0bound}
    \bar{d}_0(\vf, \vf + \vDelta)
    &\leq
    \left(\sup_s \norm{\vDelta(s)}\right) \frac{1}{L^2} e^{LT_{\text{max}}}
\end{align}
...
Since diffeomorphism cannot change the topological conjugacy class, we need the perturbation to take into account the slack.
...
\begin{align}
	...
	&\leq
	\left(\sup_s \norm{\vDelta_1(s)}\right) \frac{1}{L^2} e^{LT_{\text{max}}}
\end{align}
...
Thus, the smallest additive perturbation for a given the complexity of the diffeomorphism can be described in the following decomposition of the total perturbation as additive in vector field:
...
In neural computation, archetypes include theoretical constructs such as a ``standardized'' fixed points, limit cycles, ring attractors, and continuous attractors. These idealized systems encapsulate essential computational roles in their purest and often analytically tractable forms. For instance, an ideal ring attractor exhibits perfect rotational symmetry, free from distortion or drift. While biological and artificial neural circuits inevitably deviate from such perfection\footnote{They are non-repeating over time even for the same individual due to the ever changing brain\citep{chirimuuta2024brain}.}, treating them as deformations of these archetypes offers a powerful interpretive framework.

Constructing a dictionary of dynamical archetypes involves selecting a discrete set of theoretically grounded dynamical structures that cover a space of simple computations. Rather than inferring archetypes directly from data, we define them a priori based on theoretical insights into neural computation. Once established, this dictionary provides a reference vocabulary for analyzing observed dynamics by measuring their dissimilarity or deformation relative to these archetypes.
...
Attractors can be systematically classified by their topological dimension, with the simplest topological spaces serving as fundamental building blocks.
 In neural systems, trial-averaged activity often reflects low-dimensional structure, corresponding to an embedding of the task manifold into firing rate space \citep{gao2015simplicity}.
Recent work has shown that task-relevant computations often emerge on such manifolds \citep{langdon2023unifying,can2021emergence,cueva2021continuous,gort2024emergence,mishra2021continual,chaudhuri2019attractor,ghazizadeh2021slowmanifold,duncker2021dynamics, pezon2024linking,fortunato2024nonlinear}.
...
This perspective offers a principled framework for enumerating attractor types, which we use to define the components of our library (see Supp. Sec.~\ref{sec:library}).
\begin{enumerate}[start=0,label={\bfseries Dim \arabic*:}]
\item  Stable fixed point and fixed-point topology motifs
\item  Bounded line attractor ($[-1,1]$), ring attractor ($S_{v=0}^1$ ring with zero flow), stable limit cycle ($S_1^1$)
\item Plane attractor ($[-1,1]^2$), Cylinder attractor ($[-1,1]\times S_0^1$), Cylinder-LC ($[0,1]\times S_1^1$), Torus attractor ($(S_0^1)^2$), Torus-LC ($S_0^1\times S_1^1$), QPTA ($(S_1^1)^2$), Sphere attractor $S^2$
%\item ...
\end{enumerate}
...
%justifications: imperfect continuous attractors
...
\subsection{Two types of deformations to an ideal ring attractor}\label{sec:imp_ring}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.99\linewidth]{ring_pert_fig}
    ...
    (\textbf{B}) The complexity of the mapping from the canonical motif to the target is proportional to the complexity of the homeomorphism to deform the ring attracotr (with an offset from the rescaling of the target trajectories).
    (\textbf{C}) The second type is an additive perturbation to the vector field, which destroys the ring attractor, but the dissimilarity is small, hence the slow invariant manifold can be approximated by a ring attractor.
    (\textbf{D}) Even though the topology of the ring attractor is immediately broken, the dissimilarity is slowly growing as the fixed points along the invariant slow manifld become stronger and the perturbed ring starts to resemble a multi-fixed point system.
    }
...
Low-dimensional attractors are embedded in high-dimensional neural activity space.
Therefore, we investigate the attractors of task-trained RNNs through our canonical motif library.
We fit motifs to different sizes of RNNs trained on an angular velocity integration task (from \citet{Sagodi2024a}). %zero-input 
The ring attractor motif is able to approximate both the finite (sampled trajectories) and infinite (invariant manifold) time horizon behavior of the networks (Fig. \ref{fig:avi_rnn_recttanh}).
%high dims
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\linewidth]{avi_rnn_recttanh}
    \caption{The ring attractor motif fit to RNNs of different sizes trained on an angular velocity integration task.
...
We compare our method to the state-of-the-art methods DSA\citep{ostrow2024beyond} and SPE\citep{friedman2025characterizing}.
...
1) Ring attractors (and its perturbed and noisy versions),
2) RNNs trained on a velocity integration task,
3) limit cycles (Van der Pol oscillator, Lienard sigmoid, Sel'kov)
4) system with two bounded line attractors.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.99\linewidth]{archetype2target}
    \caption{Archetypes are evaluated based on their similarity to each target system and the complexity of the deformation required to match it.
     Square size encodes two normalized quantities: similarity (yellow) and deformation simplicity (blue).
    ...
    (\textbf{B}) Target-normalized similarity: for each archetype, it shows the targets it effectively approximates, reflecting its explanatory range.
    }
    \label{fig:archetype2target}
\end{figure}

Normalized similarity pairs target-motif that are almost topological conjugate and for which a small deformation is possible (Fig.~\ref{fig:archetype}).
...
Finally, if a target has a large dissimilarity to all the canonical motifs in our library, that means that our library is not expressible enough to cover all computation with corresponding Platonic ideals
(although it could be that our paramtrization of the homeomorphism is not expressible enough).
