...
Through abstraction and idealization, we can distill the essential features of neural trajectory, enabling meaningful understanding rather than merely presenting a detailed but opaque description.
A further abstraction, now widely adopted, is to focus on their long-term or asymptotic behavior--discretizing the continuous state space into attractors, their basins, and their connection to both the lower level implementation (e.g. circuits) and higher level information (e.g. input and context) being processed.
...
Yet, this abstraction of the mechanism is mistaken to impose identical asymptotic behavior of the ODE implementation, neglecting that systems with different asymptotic structures can exhibit indistinguishable behavior over task-relevant time scales.
...
We need to conceptually separate the ODEs and their interpretable discrete abstraction, while making the connection precise through a measure of similarity in terms of their effective behavior at the finite time scales.
...
The key concept is the \emph{topological conjugacy} (Sec.~\ref{def:top_conj}) between two systems where trajectories can be mapped between each other through a homeomorphism. % by Poincare...
