...
\usepackage{placeins}      %forcing figure placement
\usepackage{booktabs}
\usepackage{multirow}
...
\graphicspath{{figures/}{figures/all_targets}{figures/all_sources}}
...
\AtBeginDocument{\setlength{\intextsep}{9pt}} % Vertical space above & below [h] floats
\AtBeginDocument{\setlength{\textfloatsep}{9pt}} % Vertical space below (above) [t] ([b]) floats
...
\titlespacing*{\subsection}{0pt}{2pt plus 2pt minus 2pt}{0pt}
...
Our guiding principle is to group dynamical systems by their similarity in what actually matters to neural computation---their \emph{effective behavior}, and at the same time, to represent some groups by their most interpretable member.
...
\begin{figure}[t!bhp]
    ...
    \caption{Dissimilarity to an increasingly distorted ring attractor. % two types of perturbations to a ring attractor that are controlled by a scalar perturbation size. 
...
% %time repraram%
% For a given source \( \vg \)  (satisfying orbit equivalence), there exists a reparametrization of time \( \tau(t) \) such that \(\vy(\tau(t)) \approx \vx(t)\)
% with a trajectory \( \phi^t(\vy(0))_{\vg} \). %  obtained by integrating \( \dot{y} = g_i(y) \) from an initial condition \( y(0) \). 
% %So there exists $\mu(x)$ positive such that 
% %We simplify this by assuming symmetry across space $\mu\in\reals_{>0}$.
% An empirical estimator for $\bar{d}_1(\vf, \vg; \homeo^\ast)$
% :A loss function capturing this trajectory discrepancy is given by:
% \begin{equation}\label{eq:loss}
% \mathcal{L}(\theta, \mu) = \sum_{t} \Big\| (x_k(t)) - \homeo_\theta(\vy_k(t)) \Big\|^2,
% \end{equation}
...
%\mpcomment{show where we are going before switching topics so quickly.}
...
%\mpcomment{Let's put a table of archetype and corresponding theoretical model or example memory manifold structure.}
...
%\midrule
%2 & Torus                   & Coupled oscillations, grid cells, two phase-like variables \\
%2 & Sphere                  & Global integration, bounded exploration \\
\bottomrule
\end{tabular}
\end{table}
...
This research was conducted while visiting the Okinawa Institute of Science and Technology
(OIST) through the Theoretical Sciences Visiting Program (TSVP).
...
%Quantifying local distortion of the Homeomorphism
...
%
%
%\paragraph{Standard Mat\'ern Kernel (Based on Geodesic Distance)}
%Replace the Euclidean distance  $\| x - x' \|$  in the Mat\'ern kernel with the distance to the manifold  $ d_{\text{manifold}}(x, x')$. 
...
%
%\subsubsection{Imperfect coherence}%+forward invariance   /for stochastic / random systems
...
\newpage
\section{First-order bound and the Pareto frontier}\label{sec:fojac}
...
We assume an Ansatz for the diffeomorphism, under the assumption that it is close to identity:
\begin{align}\label{eq:appendix:h:ansatz}
    \vy &= \homeo(\vx) = \vx + \epsilon \tilde\homeo(\vx) + O(\epsilon^2).
\end{align}
Note that from \eqref{eq:appendix:h:ansatz}, we have
\begin{align}
    \nabla\homeo(\vx) &= \identity + \epsilon\nabla\tilde\homeo(\vx) + O(\epsilon^2).
\end{align}
By taking the time derivative of \eqref{eq:appendix:h:ansatz}, we get
\begin{align}
...
%\subsubsection{Two Bounded Line Attractors}
%BLA+ Bistable
...
%In Fig.~\ref{fig:all_archetypes_vdp}, we set $\mu = 0.3$.

%Also used in \citet{friedman2025characterizing}.
...
%\begin{figure}[htbp]
%    \centering
%    \includegraphics[width=.9\linewidth]{all_archetypes_vdp_noisy_var1_ntraj5_ai}
%    \caption{Comparison of fitting different archetypes to the noisy trajectories generated by a Van der Pol oscillator.
%     The archetypes considered in the fitting process are the ring attractor, limit cycle, and stable stable fixed point.
%     }
%    \label{fig:all_archetypes_vdp}
%\end{figure}
...
%Used in \citep{friedman2025characterizing}.

%\begin{figure}[htbp]
%    \centering
%    \includegraphics[width=.5\linewidth]{selkov_vf_trajs}
%    \caption{
%     }
...
% Used in \citep{friedman2025characterizing}. % + TWA

%\begin{figure}[htbp]
%    \centering
%    \includegraphics[width=.5\linewidth]{lienard_vf_trajs}
%    \caption{
%     }
...
\midrule
\multirow{5}{*}{Simulation for target trajectories} %add AnalyticalDS?
