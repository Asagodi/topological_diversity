...
\section{The action of the input on the attractor manifold}
...
We expand \( F(x, u) \) in a Taylor series around \( u = 0 \):
\[
F(x, u) = f(x) + \sum_i u_i g_i(x) + \sum_{i,j} u_i u_j h_{ij}(x) + \cdots,
\]
with:
\[
f(x) = -x + \phi(Wx),
\]
\[
g_i(x) = D\phi(Wx) \cdot B_i,
\]
\[
h_{ij}(x) = \frac{1}{2} B_i^\top H_\phi(Wx) B_j,
\]
where \( D\phi(Wx) \) is the Jacobian of \( \phi \) evaluated at \( Wx \), and \( H_\phi(Wx) \) is the Hessian (or second derivative tensor) of \( \phi \), applied elementwise.
