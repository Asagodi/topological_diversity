...
2) RNNs trained on a velocity integration task (with a slow ring invariant manifold),
3) limit cycles (Van der Pol oscillator, Lienard sigmoid oscillator and Sel'kov oscillator)
4) system with two bounded line attractors (to which no archetype aligns well).
...
\begin{figure}[htbp]
    ...
    \caption{Archetypes are evaluated based on their similarity to each target system and the complexity of the deformation required to match it.
     Square sizes encode two normalized quantities: similarity (yellow) and deformation simplicity (blue).
   %  (\textbf{A}) Archetype-normalized similarity: for each target, it identifies the archetype that best approximates it (large yellow square) with minimal deformation (large blue square).
     Target-normalized similarity shows which archetype best suits a target. 
    Archetype-target pairs that are near topologically conjugate will 
    }
    \label{fig:archetype2target}
\end{figure}

Archetype-normalized similarity pairs target-motif to archetypes that are almost topological conjugate and for which a small deformation is possible (Fig.~\ref{fig:archetype2target}). 
For example, 
...
Target-normalized similarity show which targets an archetype can represent.
...
\begin{figure}[htbp]
    ...
    \caption{We compare our method to the state-of-the-art methods DSA\citep{ostrow2024beyond} and SPE\citep{friedman2025characterizing}.
        (\textbf{A}) 
\textbf{(B)} Applying SPE to a homeomorphically deformed ring attractor (Fig.~\ref{fig:ring_pert_fig}A) results in inaccurate dynamics and a distorted invariant manifold.  
\textbf{(C)} SPE similarly fails on a ring attractor with an additive vector field deformation (Fig.~\ref{fig:ring_pert_fig}C), producing incorrect dynamics and a degenerate manifold structure.
    }
    \label{fig:dsa_spe}
\end{figure}
