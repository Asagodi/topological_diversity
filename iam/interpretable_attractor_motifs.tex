...
\item averaging methods \citep{sanders2007averaging} (invariant tori\citep{novaes2024invariant})
...
Then, there exists a perturbation $p\in C^1$ and a homeomorphism $\Phi:\reals^n\rightarrow\reals^n$ such that 
$\|\psi_{f+p} - \Phi(\psi_{g_i})\|_\infty = 0$.

Therefore, we can define the distance w.r.t. each motif as $d(f,g) = \inf \|p\|$ with the above constraint.
...
\subsection{Per atttractor basin motif matching}
