...
To make these ideas concrete, we organize attractors in a hierarchy based on their topological dimension. This classification provides a natural vocabulary for describing the dynamics we observe in neural systems, and serves as the foundation for our attractor library (see Supp.Sec.~\ref{sec:library}).
At dimension 0, we include fixed-point attractors, which correspond to isolated stable states — common in decision-making or memory models where the system settles into one of several discrete outcomes.
At dimension 1, we define the archetypes \emph{bounded line attractor}, which capture continuous integration over a range (e.g., position estimation and evidence integration), \emph{ring attractor}, a structure proposed for representing variables like head direction or phase and the \emph{limit cycle}, a dynamic trajectory that loops periodically, reflecting rhythmic or stereotyped behaviors.
