...
\left\|\frac{\partial \Phi - \operatorname{id}}{\partial x}\right\|_p = \left\|\frac{\partial \Phi}{\partial x} - \mathbb{I}\right\|_p
\end{equation}
...
%\item ...
\end{enumerate}
We construct  higher-dimensional attractors from fundamental topological spaces such as $S^n$ by taking their Cartesian products, see Supp.Sec.~\ref{sec:composite}.
...
%description
See for details Supp.Sec.~\ref{sec:homeopert_exp_details}.
...
%example of f
\textbf{Fully Connected Feedforward Neural Network}:
\[
...
\multirow{5}{*}{Target Homeomorphism} 
    & Type: \texttt{node} (Neural ODE) \\
    & Hidden layers: \texttt{[128]} \\
    & Activation: \texttt{ReLU} \\
    & Initialization: \texttt{random} with weight sampled from $\mathcal{N})(0.02,0.5)$ \\
\midrule
...
\multirow{5}{*}{Dynamical System \& Simulation} %add AnalyticalDS?
    & Dimension: \texttt{2}, Time step: \texttt{0.2} \\
    & Time span: \texttt{[0.0, 2.0]} \\
    & Noise standard deviation: \texttt{0.0} \\
    %& Ring vector field :\\
    %& Initial condition mode: \texttt{random} \\
    & Number of trajectories: \texttt{50} \\
    %& Margin: \texttt{0.5} \\
    %& Random seed: \texttt{42} \\
    & Train/test split ratio: \texttt{0.8 / 0.2} \\
\midrule
\multirow{3}{*}{Training} 
    & Learning rate: \texttt{0.01} \\
    & Epochs: \texttt{1000} \\
    & Batch size: \texttt{32} \\
...
\subsection{Vector field-based perturbation experiment}
