...
These two notions together motivates integrated measures such as,
\begin{align}
...
\begin{align}\label{eq:d1}
    d_1(\vf(\vx), \vg(\vx); \homeo)
	&\coloneq \int
	\norm{\phi_{\vf}^t(\vx) - \homeo(\phi_{\vg}^t(\vx))}
	\dm{t}
\end{align}
which is non-negative but non-symmetric in general.
If more complicated deformations are needed to minimize \eqref{eq:d1}, it is natural to consider their effective behavior less similar.
...
Note that the diffeomorphism preserves topological conjugacy, however, we can relax the dichotomic nature of the topological conjugacy by introducing a complexity measure.
...
\item  Bounded line attractor ($[-1,1]\subset\reals$), ring attractor ($S_{v=0}^1$ ring with zero flow), stable limit cycle ($S_1^1$)
\item Plane attractor ($[-1,1]^2$), Cylinder attractor ($[-1,1]\times S_0^1$), Cylinder-LC attractor ($[0,1]\times S_1^1$), Torus attractor ($(S_0^1)^2$), Torus-LC attractor ($S_0^1\times S_1^1$), QPTA ($(S_1^1)^2$), Sphere attractor $S^2$
%\item ...
\end{enumerate}
...
\begin{figure}[htbp]
    ...
    \caption{The ring attracotr motif fit to RNNs of different sizes trained on an angular velocity integration task.
...
We compare our method to the state-of-the-art methods DSA\citep{ostrow2023beyond} and SPE\citep{friedman2025characterizing}.
...
Although we have only analyzed working memory, the approach can be extended to input-driven, or even embo++died systems that interact with an environment.
