...
We denote this natural probability distribution over the points on the phase space as $p(\vx)$.

These two notions suggests the following dissimilarity measure:
For all $\vx \in p(\vx)$ and $t \in [0, T_\text{max}]$,
\begin{align}
    \norm{\phi^\vf_t(\vx) - \phi^\vg_t(\vx)} \leq \alpha
\end{align}
