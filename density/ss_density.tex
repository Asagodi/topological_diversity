...
Any polynomially space-bounded Turing machine can be simulated by a family of polynomial-size continuous time symmetric Hopfield nets \citep{sima2003continuous}.
...
\subsubsection{Looking beyond the finite-time horizon} %
%Why should we care?
...
\paragraph{FMP makes approximation results trivial} %put in prev section?
...
Assumption that the underlying dynamics determined by the models converge to a fixed point and neuroscience.
 models of neural computation: broader class of convergence such as oscillations \citep{townley2000existence, kag2020rnns, chang2019antisymmetricrnn}. %could convey preferred capabilities in learning 
...
\subsubsection{Alternative: isolating neighborhoods as persistent}
...
%2D DS
$\{\varphi(t,x_0) \mid x_0\in S^1\}$ forms a connected 1D manifold for each $t$
For each \( t \in \reals \), the image \( \{ \varphi(t, s) \mid s \in I \} \) remains a connected 1D manifold because:

\begin{enumerate}
    \item The initial family \( \gamma(I) \) is a 1D manifold.
...
1.2: For limit cycles this is a torus-shaped 2D manifold (in 3D, in 2D it is union of 2 $S^1$s): 


1.3: These manifolds are forward invariant for each $t$
...
\subsection{Interpretability}%move to anti-truncated koopman operator paper?
