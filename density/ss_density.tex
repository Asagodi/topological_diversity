...
\usepackage[abbrvbib]{jmlr2e} %preprint
\usepackage[utf8]{inputenc}
\usepackage[english]{babel}
\usepackage{amssymb, amsmath, amsfonts} % without amsthm
\usepackage{mathtools, mathrsfs, dsfont} % without thmtools
\usepackage{bbm}
\usepackage{forloop}
\usepackage{sidecap}
\usepackage{enumitem}
...
\usepackage{lastpage}
\jmlrheading{??}{202?}{1-\pageref{LastPage}}{??/25; Revised ??/2?}{??/2?}{21-0000}{\'Abel S\'agodi and Il Memming Park}

\ShortHeadings{UAP for infinite horizon}{S\'agodi and Park}
\firstpageno{1}
...
\begin{SCfigure}[10][bthp]
  \centering
  \includegraphics[width=0.5\textwidth]{ds_land_rnns_fmp}
...
%Neural manifold ordinary differential equations  \citep{lou2020neural}%no approximation result...
...
Finally, a new method was developed to approximate FMP systems by using a kernel representation (instead of a state-space representation) of the model  \citep{huo2024kernel}. %approximate all fading memory functionals
...
%\ascomment{TODO:} remark about LCs (neuro specific)
...
\int_{\tilde{\gamma}}\|\tilde{g}(x)\| dx = \frac{1}{T}. 	% for the integrated vector field along the limit cycle $\int_{LC}f(x)$.
\end{equation}
...
g + a_{\varepsilon_T}^*(g_{\operatorname{bump}}+ \hat{\varepsilon}\bar{g}_{\operatorname{bump}}),
 \end{equation}
where $\hat{\varepsilon}\bar{g}_{\operatorname{bump}} = g_{\operatorname{bump}} - g_{\operatorname{bump}}$ with $\|\bar g\|=1$ and $\hat \varepsilon <\max(\varepsilon_T,\varepsilon_{LC})$. %actually this should be a function of epsilon T so that closeness in Supp.Sec.~\ref{sec:periodcloseness} is guaranteed
...
%We will consider input-driven systems.  %general non-autonomous systems (Sec.~\ref{sec:generalnonautonomous}) 
...
We assume that $g(x,u(t))$ is exponentially asymptotically autonomous (EAA) dynamics for each $u$ (Def.~\ref{def:eaa_input} with $\sigma,\gamma=1$). %Supp.Sec.~\ref{sec:eea_input}
Similarly, as for the autonomous case, $g$ needs to be smooth enough as a function of $x$ and $u$ (at least Lipschitz or $C^1$). %C^1 for persistence!
Furthermore, we assume that $g$ converges to a finite number of topological equivalence classes of asymptotically autonomous vector fields. %can we increase it to countable?
...
We  characterize the input space as
$u\in \mathcal{U}$ with $u:\reals_+\rightarrow\reals^m$ with $u(t)$ converging (exponentially) to a periodic function, i.e., %or g(\dot,u) needs to (?)
%rate of convergence?
\begin{equation}
...
This guarantees exponential asymptotic autonomy for $g$. % and this assumption is minimal %SPECIFY
...
For the metric to compare input-driven systems with their approximations, see Supp.Sec.~\ref{sec:inputdriven_metric}.
...
%after T
We have that for a finite time we will make at most $\Psi(\eta,T)$ error (for a vector field that is $\eta$ close). $T$ depends on asymptotic exponent $\alpha$ at which the input-driven vector field  converges to its autonomous limit (see Def.~\ref{def:eaa_input}). %$g$ is exponentially asymptotically autonomous
...
We consider Eq.~\ref{eq:inputdriven_asymp} with $T$ for which the convergence of the non-autonomous vector field is at most $\eta$ away from the asymptotic autonomous vector field $h_{\operatorname{aut}}(x)$. %which is a combination of f(x) and the asymptotic part of g(x)
...
%Up to finite $T$: $d(p(x_1,t),\hat p(x_1,t))<\mu$. % distance?
%
...
%with $\|h_{\operatorname{error}}\|=1$ and $\kappa \ll 1$. %For $h_aut(x)$ there is a $\hat h_aut$ that is $\epsilon_h$-$\delta_h$ close.
%\begin{equation}
...
%$p_{h_{inpt}}(x,t) \rightarrow p_{h_{aut}}(x)$ for $t\rightarrow \infty$ %delta functions on attractors
...
% TODO ????? This equivalence does not hold for input-driven systems (?)  (according to  \citep{dinc2025latentcomputing} they are?) 


%%neuro specific:
\subsection{Implications for understanding neural computation} 
Recent work has demonstrated the potential of recurrent neural networks to reconstruct the latent computational dynamics underlying neural activity directly from time series data \citep{durstewitz2023reconstructing}. This approach allows for the recovery of dynamical motifs such as attractors, oscillations, and bifurcations, providing a powerful framework for inferring the computational mechanisms implemented by neural circuits.

These methodologies fall under the broader umbrella of system identification in neuroscience, which aims to recover the functional form of neural dynamics from data \citep{han2023si}. Advances in this area contribute to a deeper understanding of how neural circuits process information and generate behavior over time.
%link better to paper:
Temporal generalization—i.e., the ability of a model to extrapolate dynamics beyond the training window—is a key property of successful system identification. This capability reflects the model’s ability to capture the underlying structure of the neural system rather than merely fitting transient patterns.

%TODO?? Seperatrices are robust hence decision-making is robust to D-type noise  \citep{Park2023a}. %leave out?+ basin stability \citep{menck2013basinstability}
...
Finally, sums of polynomial functions are known to have the UAP \citep{llavona1986approximation}.
...
\begin{equation}\label{eq:node_general}
\frac{d \mathbf{h}(t)}{dt} = f(\mathbf{h}(t), t; \theta),
\end{equation}
where \( \mathbf{h}(t) \) is the hidden state at time \( t \), \( f \) is a neural network with parameters \( \theta \), and \( t \) represents time.
Given an initial condition \( \mathbf{h}(t_0) = \mathbf{h}_0 \), the evolution of the hidden state over time is computed by solving the ODE using numerical solvers, allowing the model to capture complex, continuous transformations \citep{massaroli2020nodes}.
...
Here, \( \sigma \) is an activation function (e.g., ReLU, tanh, or sigmoid), \( W_1, W_2 \) are weight matrices, and \( \mathbf{b}_1, \mathbf{b}_2 \) are bias vectors (the weights and biases together form \(\theta\) in Eq.~\ref{eq:node_general}).
This form models the ODE dynamics using a simple two-layer feedforward network.

\paragraph{UAP for NODEs}
...
\paragraph{Applications in neuroscience}
%NODEs in neuroscience%
NODEs are being increasingly adopted in computational and systems neuroscience, showing improved performance compared to current approaches  \citep{kim2021inferring,geenjaar2023learning,sedler2023expressive,elgazzar2024universal,rubanova2019latent,coelho2024enhancing}.
...
% cannot have limit cycles  in the asymptotic vector field  if there is periodic forcing.
