...
%
Then, for an arbitrary $\varepsilon, \delta > 0$, there exist a FNN $\hat f: \reals^n\rightarrow\reals^n$ with the UAP (see Sec.~\ref{sec:hypothesis}) such that the NODE defined by it $\dot x = \hat f(x)$ %just point to definition in app??
% \begin{equation}
...
%The left hand side's volume is at most $\mathcal{O}(\delta)$ with $\delta\leq\varepsilon_{\operatorname{sep}}$ the Hausdorff distance between the full separatrix submanifolds.
...
Therefore, by Eq.~\ref{eq:separatrixoverlap}, the overlap of the separatrices is less than $\mathcal{O}(\varepsilon_{\operatorname{sep}})$. %TODO: figure out bound on the volume
%compactness should imply that the volume is at most $\mathcal{\varepsilon_{\operatorname{sep}}$.
...
\tilde{f}= \hat{f}+a_{\varepsilon_T}^*\hat f_{\operatorname{bump}} = 
 ...
 \end{equation}
where $\hat{\varepsilon}\bar{f}_{\operatorname{bump}} = f_{\operatorname{bump}} -\hat f_{\operatorname{bump}}$ with $\|\bar f\|=1$ and $\hat \varepsilon <\max(\varepsilon_T,\varepsilon_{LC})$. %actually this should be a function of epsilon T so that closeness in Supp.Sec.~\ref{sec:periodcloseness} is guaranteed
...
We can find $a_i^* = a_{\varepsilon_{T_i}}$ and $\hat\varepsilon_i$ for $i=1,\dots,N$ such that Eq.~\ref{eq:lc_constraint} is satisfied for each limit cycle.
...
\begin{equation}\label{eq:inputdriven_asymp}
g(x(t), u(t)) =  h_{\operatorname{aut}}(x(t)) + \varepsilon h_{\operatorname{inp}}(x(t), u(t))
\end{equation}
...
We can approximate that with $\varepsilon''$ accuracy with our vector field.
So we are at most $\eta+\varepsilon''$ away from the actual target dynamics.
...
\textbf{What is the maximal error $g-\hat g$ we can allow for the $\varepsilon$-$\delta$ distance in flow fields to hold?}
