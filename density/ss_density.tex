...
Finally, more recently it was shown that discrete-time RNNs are finite-time horizon universal approximators of discrete-time systems \citep{aguiar2023universal}.

Universality of linear recurrences followed by nonlinear projections for finite time \citep{orvieto2023universality}.
...
We start by showing the UAP for our hypothesis class for structurally stable systems with a finite set of fixed points  (introduced in Sec.~\ref{sec:hypothesis}).
...
%TODO: introduce boa Basins of Attraction (BoA)
...
\tilde{f}= \hat{f}+\hat f_{\operatorname{bump}} = 
 \hat{f} + a_{\epsilon_T}^*(f_{\operatorname{bump}}+ \hat{\epsilon}\bar{f}),
 \end{equation}
where $\hat{\epsilon}\bar{f} = f_{\operatorname{bump}} -\hat f_{\operatorname{bump}}$ with $\|\bar f\|=1$ and $\hat \epsilon <\max(\epsilon_T,\epsilon_{LC})$. %actually this should be a function of epsilon T so that closeness in Supp.Sec.~\ref{sec:periodcloseness} is guaranteed
...
Can we perform the same procedure for $N>1$ limit cycles?
...
\tilde f_i = \hat{f}_i + \sum_{j}a(\epsilon_{T_j})\hat{\epsilon}_j\bar{f}_j.
\end{equation}
...
We need to find $a_i^* = a(\epsilon_{T_i})$ and $\hat\epsilon_i$ for $i=1,\dots,N$ such that 
...
- approximation of maximal attractor in Hausdorff sense possible
...
We will consider general non-autonomous systems (Sec.~\ref{sec:generalnonautonomous}) and input-driven systems (Sec.~\ref{sec:inputdriven}).
...
In any case we cannot have a continuous attractor in the asymptotic vector field.


%%%
-Exponentially asymptotically autonomous dynamics Supp.Sec.~\ref{sec:asyauto} \ref{def:eaa}
%
...
These have solutions $\psi(t,x_0,u)$.
...
Input doesn't need to be periodic! %but in the appropriate norm these functions will be on a period funciton...
...
%any consideration of multistable contractivity?

Needs to be hyperbolic (in the limit $t\rightarrow\infty$).

Can only have a finite set of fixed points (in the limit $t\rightarrow\infty$).
...
\subsubsection{Approximating $g(x,u)$}
 $\hat g\colon \reals^{2n} \rightarrow \reals^{n}$.

What is the maximal error $g-\hat g$ we can allow for the $\epsilon$-$\delta$ distance in flow fields to hold?
...
\|\varphi-\hat \varphi\|_\epsilon \coloneqq  \mathbb{P}\left(\|\varphi(\cdot,x_0,u)-\hat \varphi(\cdot,x_0,u)\|_\infty>\epsilon\right).
\end{equation}
...
\paragraph{Uniform (Lebesgue) Measure on a Bounded Subset of \( C([0,T], \reals^m) \)}
...
\paragraph{Stochastic dynamics} %+pdes?
\citep{wei2022statistically}


\subsection{Remarks on error}
...
\subsection{The price of universality}
...
%%%
\subsection{Semiflows}\label{sec:semiflow}
...
\subsubsection{Robustness of contractive dynamics}
...
where \( f(x, t) \) is contracting in the metric \( M(x,t) \). Then the geodesic distance \( d_{M(t)}(x_p(t), x(t)) \) satisfies the differential inequality:

\[
\frac{d}{dt} d_{M(t)}(x_p(t), x(t)) \leq -\lambda(x_p(t), t) d_{M(t)}(x_p(t), x(t)) + \|d(x_p(t), t)\|_M
\]
