...
%
Then, for an arbitrary $\epsilon, \delta > 0$, there exist a FNN $\hat f: \reals^n\rightarrow\reals^n$ with the UAP (see Sec.~\ref{sec:hypothesis}) such that the NODE defined by it $\dot x = \hat f(x)$ %just point to definition in app??
% \begin{equation}
...
The fixed points of $C^1$ ODEs can be arbitrarily closely approximated. %Hausdorff distance % in the $\|\varphi-\hat \varphi\|_\epsilon < \delta$ flow sense.
...
\begin{SCfigure}[10][bthp]
  ...
  \caption{
	...
	Trajectories initialized in the red region go to $A_i^f$ for the target system (green line) and to $A_i^g$ for the approximation (pink line). % (that is at an $\epsilon_i$ distance) 
...
where \(\overline{\cdot}\) denotes the closure of a set (same as \(\cl\)). %use consitent notation!!!!!!
...
%The left hand side's volume is at most $\mathcal{O}(\delta)$ with $\delta\leq\epsilon_{\operatorname{sep}}$ the Hausdorff distance between the full separatrix submanifolds.
...
%
By Sec.\ref{sec:separatrix_approx} (Prop.~\ref{prop:min_sep_bound}) we can make the part of the state space which will have error bigger than $\epsilon$ arbitrarily small. %for every delta
%for every epsilon
...
\begin{enumerate}
\item Per basin: $\Psi_i(\epsilon)$ so that flow is $\epsilon$ close (from previous finite time horizon + trajectory closeness to attractor after finite time or from FMP based results)
...
This implies that there exists a 3 layer FNN $\hat f_{\operatorname{bump}}$ that approximates the vector field $f_{\operatorname{bump}}$ that is a bump function around the limit cycle (similar idea as Supp.Sec.~5 in \citep{Sagodi2024a}). %but this would be an actual possible implementation
...
%CONSTRUCT:
%
The resulting total approximation is %replace a(\epsilon_T) with just an optimal constant?
...
\tilde{f}= \hat{f}+a_{\epsilon_T}^*\hat f_{\operatorname{bump}} = 
 ...
 \end{equation}
where $\hat{\epsilon}\bar{f}_{\operatorname{bump}} = f_{\operatorname{bump}} -\hat f_{\operatorname{bump}}$ with $\|\bar f\|=1$ and $\hat \epsilon <\max(\epsilon_T,\epsilon_{LC})$. %actually this should be a function of epsilon T so that closeness in Supp.Sec.~\ref{sec:periodcloseness} is guaranteed
%no this is not the bound that belongs to hat epsilon!!!!!!!!!!!
%it should be \|f_{\operatorname{bump}\| < some epsilon...
%
%So if the period gets longer because of the perturbation (i.e. $\hat \ell(\epsilon)$ is an increasing function) as long as $1/(\hat \ell+g(\epsilon))$ is not decreasing at the same rate there should exist a value $\epsilon$ for which the equality holds.
...
\end{equation}
%TODO: replace a(\epsilon_{T_j}) with a_j^*? 
...
%we have to guarantee that a(\epsilon_{T_i})
...
Each of the neighboring stable fixed points can be at most $\epsilon$ apart. %needs to be smaller because of $\mathcal{O}(\epsilon)$ dispacement of stable FP of approx?
...
\subsubsection{Targets: Constraints on $g$}
...
We assume that $g(x,u(t))$ is exponentially asymptotically autonomous dynamics for each $u$ (Def.~\ref{def:eaa_input}). %Supp.Sec.~\ref{sec:eea_input}
...
\paragraph{Include a measure-zero set of input sequences that lead to non-hyperbolic asymptotically autonomous vector fields}
...
\subsubsection{Hypothesis space: Approximating $g(x,u)$} %with input-driven MLP-NODEs
...
\subsection{Theorem}
\begin{theorem}
For an input-driven system as in Eq.~\ref{eq:inputdriven} %+assymptions...
...
\end{enumerate}
\item Characterize proportion of “bad” input sequences through path integral (e.g., non-hyperbolic dynamics)
\end{enumerate}
...
\paragraph{$g$ is asymptotically autonomous} %+exponentially
We have that for a finite time we will make at most $\epsilon'$ error. %for a vector field that is .... close 
...
We consider Eq.~\ref{eq:init_path_density} with $T$ for which the convergence of the non-autonomous vector field is at most $\eta$ away from the asymptotic autonomous vector field $f'(x)$. %which is a combination of f(x) and the asymptotic part of g(x)
...
This would lead to the requirement ``$g$ needs to converge to countably many topological equivalence classes of asymptotically autonomous vector fields''
...
\]
where \( g, h \in C^\sigma \) and \( g(t,x) \to h(x) \) in the following sense: for each compact set \( K \subset \mathbb{R}^n \) and each \( \epsilon > 0 \), there exists \( T \) such that  
\[
\|\partial^\gamma g(t,x) - \partial^\gamma h(x)\| e^{\alpha t} < \epsilon
\]
...
\]
where \( g, h \in C^\sigma \) and \( g(u(t),x) \to h(x) \) in the following sense: for each compact set \( K \subset \mathbb{R}^n \) and each \( \epsilon > 0 \), there exists \( T \) such that  
\[
\|\partial^\gamma g(u(t),x) - \partial^\gamma h_u(x)\| e^{\alpha t} < \epsilon
\]
for all multi-indices \( \gamma \) with \( |\gamma| \leq \sigma \), for all \( x \in K \), and for all \( t > T \).
\end{definition}
...
%\paragraph{The effect of the correction $\tilde \epsilon\coloneqq a(\epsilon_T)\hat\epsilon$}
%- Tubular neighbourhood: $\|\hat f_{\operatorname{bump}}\|$ effect (relevant for $\overline\boa(A_i)$). 
%
%- Outside $\tilde\epsilon$ effect for a non-compact activation function (e.g., affects location of fixed points and limit cycles)
%
%-  $\tilde\epsilon$ needs to be small enough so that effect of correction outside of the tubular neighborhood doesn't affect attractors and separatrices too much.
%
...
%We have that $\|\eta\bar f  + a_{\epsilon_T}\hat \epsilon\bar f_{\operatorname{bump}}\| < \eta + a_{\epsilon_T}\hat \epsilon$ (triangle inequality and absolute homogeneity of norm). 
