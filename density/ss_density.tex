...
Then, for an arbitrary $\varepsilon, \delta > 0$, there exist a FNN $\hat f: \reals^n\rightarrow\reals^n$ with the UAP (see Sec.~\ref{sec:hypothesis}) such that the NODE defined by it $\dot x = g(x)$
% \begin{equation}
...
\item So if we have $\| f - \hat f\|<\tfrac{1}{k}\delta$ then $\vol(\widehat{\operatorname{sepmis}})<\delta$
\end{enumerate}
