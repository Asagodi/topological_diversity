...
\hline
 &  Time interval & FMP & Finite size							 			 &  Input     &     Discrete/Continuous   \\\hline\hline
\citet{cybenko1989approximation} & Finite & No 			& Yes 					 &  &\\\hline
\citet{funahashi1993approximation} & Finite & No			 & Yes				 	 &  &\\\hline
\citet{hornik1989multilayer} & Finite & No 				& Yes					 & &\\\hline
\citet{bai2019deq} & Infinite & Yes					 & No				 	 & &\\\hline
\citet{schafer2007uap} & Infinite & Yes					 & No				 	 & &\\\hline
\end{tabular}
\end{table}
...
Neural ODEs (NODEs) and Invertible neural networks (INNs) are a sup-universal approximator for a fairly large class of diffeomorphisms, i.e., smooth invertible maps with smooth inverse \citep{teshima2020uap, teshima2020coupling, ishikawa2023universal}.
...
The concept of fading memory (FM) has been a cornerstone in the study of dynamical systems, providing a framework where the system’s output depends on past inputs with gradually diminishing influence \citep{boyd1985fading}.
...
If N does have a realization as a dynamical system, then the fading memory property is related to the unique steady-state property for dynamical systems \citep{chua1976qualitative}.
The fading memory assumption implies that the state will be “asymptotically independent” of the initial condition.
\end{remark}
...
State Space Models (SSMs) with layer-wise nonlinearity are universal approximators with exponential decaying memory \citep{wang2024state}.
...
UAP of RNNs for uniformly asymptotically incrementally stable systems on an infinite time horizon\citep{hanson2020universal}.
...
\subsubsection{FMP makes DSs trivial} %put in prev section?
...
Simple Cycle Reservoirs are Universal \citep{li2023simple}
...
%
Therefore, by Fenichel's Persistent manifold theorem, there exists a $\delta_{ij}$ such that for all $g$ that are $\delta_{ij}$ $C^1$ close to $f$ such that $g$ has an normally hyperbolic invariant manifold that is diffeomorphic to and at most at $\delta_{ij}$ Hausdorff distance from $\sep(A_i,A_j)$.
...
We get a bunch of $\epsilon_j$'s and $\delta_i$'s.
Take $\epsilon_i<$.??
Then $\delta_i< \frac{\delta}{N}$ for each $i$ and hence $\sum_i\delta_i<\delta$.
\end{proof}
...
\subsection{Approximating limit cycles}
%
%
...
- Tubular neighbourhood: $\epsilon_1$ effect

- Outside $\epsilon_2$ effect
...
%%%QPTA
\paragraph{QPTA}%more generally: doing this for multiple limit cycles
\citep{Park2023a}
...
Homeomorphisms \citep{zhang2020approximation}

Universal approximation of monotone analytic functions homotopic to the identity\citep{tabuada2020universal}

Invariant functions\citep{li2022deep}
...
\begin{theorem}[\cite{robbin1971ss}, \cite{robinson1974ss}]
...
\subsection{Boundaries of basins of attraction}
The boundary of a basin of attraction is composed of points that do not belong to the basin of the periodic orbit but are arbitrarily close to it. Any trajectory starting exactly on this boundary will not enter the basin of the periodic orbit nor escape to another basin.
