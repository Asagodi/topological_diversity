...
\[\varepsilon_{\operatorname{basin}} \coloneqq \sum_{i < j}\boa(A_i^f)\cap\boa(A_j^g).\] %prove?
...
The right hand side's volume is at most $\mathcal{O}(\delta)$ with $\delta\leq\epsilon_{\operatorname{sep}}$ the Hausdorff distance between the total separatrix submanifolds.
...
Take $\delta_i< \frac{\delta}{N_{\operatorname{attractors}}}$ for each $i$ and hence $\sum_i\delta_i<\delta$.
...
We will typically have a difference between the period of limit cycle of the target $T$ and of the approximation $\hat T$: $\epsilon_T\coloneqq T-\hat T>0$
...
Then we know that the this NHIM will persist (from Fenichel's PMT).
...
%continuum of attractors (nonzero flow in at least one dim): e.g. torus
