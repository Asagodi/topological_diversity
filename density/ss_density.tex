...
We conclude with how these results extend to a class of non-autonomous dynamical systems and some remarks about approximation of chaotic orbits (Sec.~\ref{sec:chaos}).
...
\paragraph{Separatrices are persistent with isolating neighborhoods that fill the basin of attraction}
...
\tilde f_i = \hat{f}_i + \sum_{j}a_{\epsilon_{T_j}}\hat{\epsilon}_j\bar{f}_j.
\end{equation}
...
We need to find $a_i^* = a_{\epsilon_{T_i}}$ and $\hat\epsilon_i$ for $i=1,\dots,N$ such that 
