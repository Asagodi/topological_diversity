...
\item Pick asy aut vf $h_u$ for approximation (so we have $\hat h_u = h_u + \kappa \bar h_u$)
...
\paragraph{$g$ is exponentially asymptotically autonomous} %+exponentially
We have that for a finite time we will make at most $\Psi(\eta,T)$ error (for a vector field that is $\eta$ close).
