...
We can now rescale the perturbation through a scalar $a$ that depends on $\epsilon_T$ so that the period of approximation limit cycle matches the target's (Eq.~\ref{eq:lc_correction}). 
...
\tilde{f}= \hat{f}+\hat f_{\operatorname{bump}} = 
 \hat{f} + a(\epsilon_T)(f_{\operatorname{bump}}+ \hat{\epsilon}\bar{f}),
 \end{equation}
...
We need to find $a(\epsilon_{T_i})$ and $\hat\epsilon_i$ for $i=1,\dots,N$ such that 
...
\section{Non-autonomous dynamics}
...
%prove:
Finite-time horizon approximation + exponentially asymptotically autonomous = Infinite-time horizon approximation
