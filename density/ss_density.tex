...
\begin{SCfigure}[10][bthp]
  \centering
  \includegraphics[width=0.4\textwidth]{ds_land_rnns_fmp}
...
We assume that $u(t)$ converges to a periodic function, i.e., %or g(\dot,u) needs to (?)
%rate of convergence?
\begin{equation}
...
g(x(t), u(t)) \rightarrow h_{aut}(x(t))
\end{equation}
...
\paragraph{$g$ needs to converge to countably many separated asymptotically autonomous vector fields}
...
\subsubsection{Defining the metric for the approximation of non-autonomous systems}
%Reconsider $\delta$:
...
\ascomment{TODO: define $d_\epsilon$}


\subsubsection{Theorem}
\begin{theorem}
For an input-driven dynamical system as in Eq.~\ref{eq:inputdriven} %+assymptions...
...
\paragraph{Practicalities: The price of universality}
This construction is immensely inefficient.
%Better to just look at RNNs (?)
%Expressivity paper?

%%
Relation to MSE and total integrated error, see Supp.Sec.\ref{sec:error_remarks}.
...
where \( f(x, t) \) is contracting in the metric \( M(x,t) \).
 Then the geodesic distance \( d_{M(t)}(x_p(t), x(t)) \) satisfies the differential inequality:
\[
